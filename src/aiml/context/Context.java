...
/**
 * This class represents information about a single context - it's name,
...
public abstract class Context
  ...
  /** The order (or position) of this context. This is a WORM value (write once read many). */
  private int order = -1;
   ...
   *
   * <p><i>Note to self:</i>Would it be better if the order was set
   * automatically at creation time with the use of a static counter?</p>
   ...
   * @param c Object
   * @return -1 if this context has a lower order; 0 if they have the same order; 1 if this context has a bigger order
   */
   ...
   * @param c Object
   * @return <code>true</code> if both contexts have the same name and order; <code>false</code> otherwise
   */
  public boolean equals(Object c) {
    return (c instanceof Context)
        && ( ( (Context) c).getName() == name)
        && ( ( (Context) c).getOrder() == order);
  }

  /**
   * Get the value currently associated with the context. This method must be
   * overriden in inherited classes to provide meaningfull behaviour.
