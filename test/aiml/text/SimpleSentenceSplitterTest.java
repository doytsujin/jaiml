...
public class SimpleSentenceSplitterTest extends TestCase {
  ...
  protected void setUp() throws Exception {
    InputStream in = this.getClass()
        .getResourceAsStream("/aiml/text/rules.txt");
    String rules = getContents(in);
    splitter = new SimpleSentenceSplitter();
  }
  ...
  public void testPunctuation() {
    Iterator<String> sentence;
    sentence = splitter
        .split("he's vanished! what will we do?  it's up to us.").iterator();
  ...
  public void testSplitNumbers() {
    ...
    assertEquals("Please add 1.", sentence.next());
    assertEquals("5 liters to the tank.", sentence.next());
    assertFalse(sentence.hasNext());
  }
  ...
  public void testSplitFakePunctuation() {
    Iterator<String> sentence;
    sentence = splitter.split("\"No man is an island . . . every man . . . \"")
        .iterator();
    assertEquals("\"No man is an island .", sentence.next());
    assertEquals(" .", sentence.next());
    assertEquals(" .", sentence.next());
    assertEquals(" every man .", sentence.next());
    assertEquals(" .", sentence.next());
    assertEquals(" .", sentence.next());
    assertEquals(" \"", sentence.next());
    assertFalse(sentence.hasNext());
  ...
  public void testSplitAbbreviations() {
    ...
    assertEquals("My friend, Mr.", sentence.next());
    assertEquals(" Jones, has a new dog.", sentence.next());
